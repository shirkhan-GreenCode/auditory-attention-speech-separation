...
% anjame clusterin be ravesh sevom (clustering har fram).ravesh 
% estefade az marakeze frame ghabl va emkan adam vojoode iek gooiande dar iek fram
[m,n]=size (rave);
cl_total=zeros(m,1);
...
for i=2:m/10
    ...
    [r2,c2]=find(a==min(min(a)));
        ...
        Ctotal(2,:)=center(1,:);
            ...
            clx(:,1)=1;
            Ctotal(1,:)=(center(1,:)+center(2,:))/2;
                    else if r1*r2==12
                            clx(:,1)=2;
                            Ctotal(2,:)=(center(1,:)+center(2,:))/2;
                        else 
                          Ctotal(1,:)=(center(1,:));
                          Ctotal(2,:)=(center(2,:));
                        end

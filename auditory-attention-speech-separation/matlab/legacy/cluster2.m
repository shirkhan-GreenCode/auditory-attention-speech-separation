function [cl,Ctotal]=cluster3(rave,K);
% anjame clustering be ravesh dovom (clustering har fram).ravesh 
% miangin giri bad az har fram va emkan adam vojoode iek gooiande dar iek fram
[m,n]=size (rave);
cl_total=zeros(m,1);
...
for i=2:m/10
    ...
    [r2,c2]=find(a==min(min(a)));
        ...
        Ctotal(2,:)=(center(1,:)+Ctotal(2,:))/2;
            ...
            clx(:,1)=1;
            Ctotal(1,:)=(center(1,:)+center(2,:)+Ctotal(1,:))/3;
                    else if r1*r2==12
                            clx(:,1)=2;
                            Ctotal(2,:)=(center(1,:)+center(2,:)+Ctotal(2,:))/3;
                        else 
